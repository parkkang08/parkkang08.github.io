...
const startSound = new Audio("/static/start.mp3"); // 휴식 시작 / 공부 종료
const endSound   = new Audio("/static/end.mp3");   // 공부 시작 / 휴식 종료
...
function unlockAudio() {
  const p1 = startSound.play().then(()=>{ startSound.pause(); startSound.currentTime = 0; }).catch(()=>{});
  const p2 = endSound.play().then(()=>{ endSound.pause(); endSound.currentTime = 0; }).catch(()=>{});
...
function updateUI(score) {
  ...
  if (rounded > maxScore) {
    maxScore = rounded;
    maxScoreEl && (maxScoreEl.textContent = `최고 집중도: ${maxScore}%`);
  }

  scoreEl.textContent = `집중도: ${rounded}% (평균: ${sessionAvg}%)`;
...
function handleStateAndAlerts(score) {
  ...
  lastState = newState;
  stateEl.textContent = `현재 상태: ${newState === 'focus' ? '집중' : '비집중'}`;
  ...
  if (!isFocus) {
    ...
    if (elapsed >= HOLD_SEC) {
      ...
      if (Notification && Notification.permission === 'granted' && alarmOn) {
        new Notification('집중력 낮음', { body: '자세 바로잡기 / 눈 휴식' });
      }
      belowSince = now;
    }
  } else {
...
function playStartSound(){
  try { endSound.pause(); } catch(e){}
  startSound.currentTime = 0;
  startSound.play().catch(()=>{});
}
function playEndSound(){
  try { startSound.pause(); } catch(e){}
...
function updateTimerUI() {
  updateModePill();
  countdownEl.textContent = `남은 시간: ${formatMMSS(remainingSec)}`;
  const elapsed = sessionStart ? Math.floor((Date.now() - sessionStart.getTime())/1000) : 0;
  elapsedEl.textContent = `경과 시간: ${formatHHMMSS(elapsed)}`;
}
function updateModePill() {
  modeTextEl.textContent = `모드: ${mode === 'study' ? '공부' : mode === 'break' ? '휴식' : '-'}`;
...
function showBanner(message, type='ok') {
  ...
  showBanner._t = setTimeout(()=>{ bannerEl.style.display = 'none'; }, 2500);
}
